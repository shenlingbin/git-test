...
<div class="main-wrapper">
    ...
    <% PageInfo pi=(PageInfo)request.getAttribute("personalOrders"); %>
    <% List<Orders> list=pi.getList(); %>
    <div class="main-wrapper">
        <div class="container">
            <span><h2>订单信息</h2></span>
            <span>
		    <form class="example" action="customerSearchOrders.do" style="margin:auto;max-width:300px">
            ...
            <table class="table table-hover">
                ...
                <tr>
                    ...
                    <td><a href="UpdateOrder.do?id=<%=list.get(i).getId() %>"><button type="button" class="btn btn-outline-success">编辑</button></a>
                        <a href="DeleteOrder.do?id=<%=list.get(i).getId() %>"><button type="button" class="btn btn-outline-danger">删除</button></a>
                    </td>
                </tr>
                <%} %>
                </tbody>
            ...
            <div align="center" class="bg container">
                <ul class="pagination">
                    ...
                    <%if(pi.getPrePage()>0){ %>
                    <li class = "page-item"><a href="cddxx.do?ym=<%=pi.getPrePage()%>" class="page-link" aria-lable="Previous">&laquo;</a></li>
                    <%} %>
                    <% for(int i:pi.getNavigatepageNums()){ %>
                    <li class = "page-item"><a href="cddxx.do?ym=<%=i%>" class="page-link">&nbsp;<%=i %>&nbsp;</a></li>
                    <%} %>
                    <!-- 判断当前页是否存在下一页，不存在则不显示下一页的按钮 -->
                    <% if(pi.getPageNum()<pi.getLastPage()){ %>
                    <li class="page-item"><a href="cddxx.do?ym=<%=pi.getNextPage()%>" class="page-link" aria-lable="Next" >&raquo;</a></li>
                    <%} %>
                </ul>
            </div>
        </div>
